...
function AnalyticsTab({ users }) {
  ...
  const fetchErrorData = async () => {
    try {
      ...
      if (attempts) {
        // Merge local recent mistakes if Supabase has fewer
        const allAttempts = [...attempts, ...recentMistakes.map(m => ({
          id: `local-${m.timestamp}`,
          exercise_id: m.exercise_id,
          exercise_type: m.exercise_type,
          is_correct: false,
          user_answer: m.wrong_answer,
        ...
        const totalErrors = allAttempts.filter(a => !a.is_correct).length
        const errorRate = totalAttempts > 0 ? Math.round((totalErrors / totalAttempts) * 100) : 0

        // Calculate common mistakes from local store if Supabase returns nothing
        let commonMistakesList = mistakes || []
        if (commonMistakesList.length === 0 && recentMistakes.length > 0) {
          const mistakeMap = {}
          recentMistakes.forEach(m => {
            const key = `${m.exercise_id}-${m.wrong_answer}`
            if (!mistakeMap[key]) {
              mistakeMap[key] = {
                exercise_id: m.exercise_id,
                correct_answer: m.correct_answer,
            ...
            mistakeMap[key].occurrence_count++
          })
          commonMistakesList = Object.values(mistakeMap).sort((a, b) => b.occurrence_count - a.occurrence_count)
        }
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-bg-card rounded-xl p-5 border border-border-main">
              ...
              <p className="text-text-alt text-sm">Total Users</p>
            </div>
            <div className="bg-bg-card rounded-xl p-5 border border-border-main">
              <div className="w-10 h-10 bg-brand-primary rounded-xl flex items-center justify-center mb-3">
                <Activity className="text-white" size={20} />
              </div>
              <p className="text-2xl font-bold text-text-main">{dailyActive}</p>
              <p className="text-text-alt text-sm">Active Today</p>
            </div>
            <div className="bg-bg-card rounded-xl p-5 border border-border-main">
              <div className="w-10 h-10 bg-purple-500 rounded-xl flex items-center justify-center mb-3">
                <Calendar className="text-white" size={20} />
              </div>
              <p className="text-2xl font-bold text-text-main">{monthlyActive}</p>
              <p className="text-text-alt text-sm">Active This Month</p>
            </div>
            <div className="bg-bg-card rounded-xl p-5 border border-border-main">
              ...
              <p className="text-text-alt text-sm">Completion Rate</p>
            </div>
          </div>
                ...
                return (
                  <div key={item.lang} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-text-main capitalize font-medium">{item.lang}</span>
                      <span className="text-text-alt">{item.count} users ({item.percentage}%)</span>
                    </div>
                    <div className="h-2 bg-bg-alt rounded-full overflow-hidden">
                      <div className={`h-full ${colors[index % colors.length]} rounded-full`} style={{ width: `${item.percentage}%` }} />
                    </div>
                  </div>
                )
              }) : <p className="text-text-alt text-center py-4">No language data yet</p>}
            </div>
          </div>
          ...
          <div className="bg-bg-card rounded-xl border border-border-main p-6">
            ...
            <div className="space-y-3">
              {users.slice(0, 5).map(user => (
                <div key={user.id} className="flex items-center justify-between p-3 bg-bg-alt rounded-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-text-alt text-sm">{user.learning_language || 'No language'}</p>
                    </div>
                  </div>
                  <p className="text-text-alt text-sm">{new Date(user.created_at).toLocaleDateString()}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-bg-card rounded-xl p-5 border border-border-main">
              ...
              <p className="text-2xl font-bold text-text-main">{errorData.totalAttempts}</p>
              <p className="text-text-alt text-sm">Total Attempts</p>
            </div>
            <div className="bg-bg-card rounded-xl p-5 border border-border-main">
              <div className="w-10 h-10 bg-red-500 rounded-xl flex items-center justify-center mb-3">
                <XCircle className="text-white" size={20} />
              </div>
              <p className="text-2xl font-bold text-text-main">{errorData.totalErrors}</p>
              <p className="text-text-alt text-sm">Total Errors</p>
            </div>
            <div className="bg-bg-card rounded-xl p-5 border border-border-main">
              <div className="w-10 h-10 bg-brand-primary rounded-xl flex items-center justify-center mb-3">
                <CheckCircle className="text-white" size={20} />
              </div>
              <p className="text-2xl font-bold text-text-main">{errorData.totalAttempts - errorData.totalErrors}</p>
              <p className="text-text-alt text-sm">Correct Answers</p>
            </div>
            <div className="bg-bg-card rounded-xl p-5 border border-border-main">
              <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center mb-3">
                <AlertCircle className="text-white" size={20} />
              </div>
              <p className="text-2xl font-bold text-text-main">{errorData.errorRate}%</p>
              <p className="text-text-alt text-sm">Error Rate</p>
            </div>
          </div>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {errorData.commonMistakes.map((mistake, i) => (
                      <tr key={i} className="border-b border-border-main hover:bg-bg-alt">
                        ...
                        <td className="py-3 px-4 text-text-alt capitalize">{mistake.exercise_type}</td>
                        <td className="py-3 px-4">
                          <span className="px-2 py-1 bg-red-100 text-red-600 rounded-lg text-sm font-bold">{mistake.occurrence_count}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {errorData.userErrors.length > 0 ? (
              <div className="space-y-3">
                {errorData.userErrors.map(userError => {
                  ...
                  return (
                    <div key={userError.userId} className="flex items-center justify-between p-4 bg-bg-alt rounded-xl">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-amber-500 rounded-full flex items-center justify-center">
                          <span className="text-white font-bold">{user?.username?.[0]?.toUpperCase() || '?'}</span>
                        </div>
                        <div>
                          <p className="text-text-main font-medium">{user?.username || 'Unknown User'}</p>
          ...
          <div className="bg-bg-card rounded-xl border border-border-main p-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border-main">
                    <th className="text-left py-3 px-4 text-text-alt font-medium">Rank</th>
                    <th className="text-left py-3 px-4 text-text-alt font-medium">User</th>
                    <th className="text-left py-3 px-4 text-text-alt font-medium">XP</th>
                    <th className="text-left py-3 px-4 text-text-alt font-medium">Streak</th>
                    <th className="text-left py-3 px-4 text-text-alt font-medium">Lessons</th>
                  </tr>
                </thead>
                <tbody>
                  {users.sort((a, b) => (b.xp || 0) - (a.xp || 0)).slice(0, 10).map((user, index) => (
                    <tr key={user.id} className="border-b border-border-main hover:bg-bg-alt">
                      <td className="py-3 px-4">
                        {index < 3 ? (
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${index === 0 ? 'bg-amber-500' : index === 1 ? 'bg-gray-400' : 'bg-orange-400'}`}>
                            <span className="text-white font-bold text-sm">{index + 1}</span>
                          </div>
                        ) : <span className="text-text-alt font-medium pl-2">#{index + 1}</span>}
                      </td>
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                      ...
                      <td className="py-3 px-4"><span className="text-amber-500 font-bold">{(user.xp || 0).toLocaleString()}</span></td>
                      <td className="py-3 px-4">
                        <span className="flex items-center gap-1 text-orange-500">
                          <Flame size={16} /><span className="font-bold">{user.streak || 0}</span>
                        </span>
                      </td>
                      <td className="py-3 px-4 text-text-alt">{user.completed_lessons?.length || 0}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {[
                ...
                return (
                  <div key={range.label} className="flex items-center gap-4">
                    <span className="text-text-alt text-sm w-24">{range.label}</span>
                    <div className="flex-1 h-6 bg-bg-alt rounded-lg overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-orange-500 to-amber-400 rounded-lg flex items-center justify-end pr-2" style={{ width: `${percentage}%`, minWidth: count > 0 ? '40px' : '0' }}>
